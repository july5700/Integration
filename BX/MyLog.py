...
class OutPutMode(object):
    ...
    log_open = 1  # 开启log
    log_to_file = 2  # 输出到文件
    log_to_stdout = 4  # 输出到控制台


class MyLog(object):
    def __init__(self, debug_mode=7, log_folder='output'):
    ...
    def create_log_sample(self):
        ...
        match self.debug_mode:
            case 3:
                ...
                # print(f"self.debug_Mode is {self.debug_mode}")
                self.create_log_folder()
                log.remove()
                log.add(self.log_path, format="{time:YYYY-MM-DD HH:mm:ss.SSS} | {level} | {name}: {function}:{line} - "
                                    "{message}", level="DEBUG", backtrace=True, diagnose=True)
                log.add(sys.stdout)
                return log
            case 2:
                # print(f"self.debug_Mode is {self.debug_mode}")
                ...
                log.remove()
                log.add(self.log_path, format="{time:YYYY-MM-DD HH:mm:ss.SSS} | {level} | {name}: {function}:{line} - "
                                              "{message}", level="INFO", backtrace=True, diagnose=True)
                # log.add(sys.stdout)

                return log
            case 1:
                log.add(sys.stdout, format="{time:YYYY-MM-DD HH:mm:ss.SSS} | {level} | {name}: {function}:{line} - "                                            "{message}", level="INFO", backtrace=True, diagnose=True)
                return log
            case 0:
                log.remove()
                return log
            case _:
                print(f"self.debug_Mode is {self.debug_mode}")
                print("查看log level规则，并选择一个level")
...
if __name__ == "__main__":
    ...
    # l.debug_mode=3
    b = l.log_sample
    print(f"l.debug_mode is {l.debug_mode}")
    folder = l.log_folder
    log_path = l.log_path
